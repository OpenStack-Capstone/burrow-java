...
abstract class AsyncClientTest extends TestCase {
   ...
   */
  public void testDeleteQueuesWithDetail() throws ExecutionException, InterruptedException, ExecutionException {
   ...
   */
  public void testMultipleUpdateHide() throws ExecutionException, InterruptedException, ExecutionException {
   ...
   */
  public void testUpdateHide() throws ExecutionException, InterruptedException, ExecutionException {
    String ids[] = {"testUpdateHideMessage"};
    String body = "testUpdateHideMessageBody";
    boolean[] seen;
    backend.executeAsync(queue.createMessage(ids[0], body).withHide(99999));
    // TODO: Remove when getMessages no longer 404s on queues with only hidden
    // messages!
    backend.executeAsync(queue.createMessage("404workaround", "404workaround"));
    seen = scanMessages(backend.executeAsync(queue.getMessages()).get(), ids);
